...
import * as svgDef from './svg'
import { sortData } from '../Templates'

export type FindingTemplatesOptions = {
  svg?: { [key: string]: string }
}

export const createFindingTemplates = (
  options: FindingTemplatesOptions = {}
): { [key: string]: any } => {
  const svg = Object.assign({ ...svgDef }, options.svg || {})
  ...
  return {
    ...
    [FINDING_RESUME]: (doc: MdDoc, fieds?: string[] | undefined) => {
      ...
      const data = Object.entries(
        getFindingResumeData(getFindings(doc))
