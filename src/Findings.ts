...
const validateValues = (value: string, values: { low?: number; medium?: number; }, def: string)  => Object.keys(values).includes(value) ? value : def;
const validateImpact = (value: string, def = HIGH) => validateValues(value, IMPACT, def);
...
export const calculateTotalRisk = ({ impact, likelihood } : {impact: string, likelihood: string}) => {
  ...
  likelihood = flippedLikelihood[likelihoodRate as keyof typeof flippedLikelihood];
...
export const parseFinding = (data : { impact: any; likelihood: any, fixed?: boolean}) => {
